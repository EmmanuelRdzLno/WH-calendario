...
const swaggerOptions = {
  definition: {
    ...
    info: {
      ...
      version: '1.0.0',
      description: 'Simulador y documentación para el endpoint del Webhook de Google Calendar.',
    },
...
app.post('/webhook/google-calendar', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (rows.length > 0) {
        lastSyncToken = rows[0].sync_token;
      }
    } catch (err) {
    ...
    let swaggerResponse = updatedEvents.length > 0
    ...
    if (response.data.nextSyncToken) {
      ...
      try {
        await axios.post(
          'https://api-database-577166035685.us-central1.run.app/api/ejecutar-directo',
          {
